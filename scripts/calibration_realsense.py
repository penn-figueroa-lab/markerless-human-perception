...
def callback(data_color,data_depth, camera_info, m0, m1, m2, m3, m4):
    ...
    if depth_array is None:
        ...
        np.save(abs_path + "markerless-human-perception/src/depth",np.divide(depth_array,zero_counter))
        np.save(abs_path + "markerless-human-perception/src/K",K)
        np.save(abs_path + "markerless-human-perception/src/points",point_array/counter)
        np.savetxt(abs_path + "markerless-human-perception/src/depth.csv",np.divide(depth_array,zero_counter) , delimiter=',')
        np.savetxt(abs_path + "markerless-human-perception/src/K.csv",K                       , delimiter=',')
        np.savetxt(abs_path + "markerless-human-perception/src/points.csv",point_array/counter, delimiter=',')
    ...
    cv2.imwrite(abs_path + "markerless-human-perception/src/depth.png",cv2.cvtColor(cv_depth,cv2.COLOR_BGR2RGB))
    cv2.imwrite(abs_path + "markerless-human-perception/src/color.png",cv2.cvtColor(cv_color,cv2.COLOR_BGR2RGB))
